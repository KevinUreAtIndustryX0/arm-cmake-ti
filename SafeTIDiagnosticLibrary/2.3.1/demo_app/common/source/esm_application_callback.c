...
boolean pll1_slip_faultinject_callback = FALSE;
boolean pll2_slip_faultinject_callback = FALSE;
#endif
...
/********************************************************************************************************/
    ...
    if ((grp_channel & ESM_GRP1_MASK) == ESM_GRP1_MASK)
    {
    	...
    	else if(ESM_G1ERR_PLL1_SLIP == channel)
    	{
    		...
    		sl_esmREG->SR1[0] |= GET_ESM_BIT_NUM(ESM_G1ERR_PLL1_SLIP);
    		pll1_slip_faultinject_callback = TRUE;
    	}
    	else if(ESM_G1ERR_PLL2_SLIP == channel)
    	{
    		callbkParam1 = param1 & (uint32)0x1u;
    		callbkParam2 = param1 & (uint32)0x2u;
    		sl_esmREG->SR4[0] |= GET_ESM_BIT_NUM(ESM_G1ERR_PLL2_SLIP);
    		pll2_slip_faultinject_callback = TRUE;
    	}
	...
	else if((ESM_GRP1_MASK == ((grp_channel&ESM_GRP1_MASK)))&&(ESM_G1ERR_PLL1_SLIP == (grp_channel & 0x0000ffff)))
	{
		callbkParam1 = param1 & (uint32)0x1u;
		callbkParam2 = param1 & (uint32)0x2u;
		sl_esmREG->SR1[0] |= GET_ESM_BIT_NUM(ESM_G1ERR_PLL1_SLIP);
	...
	else if((ESM_GRP1_MASK == ((grp_channel&ESM_GRP1_MASK)))&&(ESM_G1ERR_PLL2_SLIP == (grp_channel & 0x0000ffff)))
	{
		callbkParam1 = param1 & (uint32)0x1u;
		callbkParam2 = param1 & (uint32)0x2u;
		sl_esmREG->SR4[0] |= GET_ESM_BIT_NUM(ESM_G1ERR_PLL2_SLIP);
