...
void main()
{
	...
	uint32			    tempVal;
	uint32 i,j;
	uint64 crcAtInit_VIMRAM;
	uint64 crcAtInit_FLASH;
...
#ifdef __TI_COMPILER_VERSION__
        if (SL_CRC_Calculate((uint64 *)((uint32)&ulFlashStartAddr), ((((uint32)&ulFlashEndAddr)-((uint32)&ulFlashStartAddr)) >> 3)) == crcAtInit_FLASH) {
           	periodicSTPassCount++;
        } else {
        	periodicSTFailCount++;
        }
...
#ifdef __TI_COMPILER_VERSION__
        if (SL_CRC_Calculate((uint64 *)(((uint32)&ulFlashStartAddr) | 0x20000000u), ((((uint32)&ulFlashEndAddr)-((uint32)&ulFlashStartAddr)) >> 3)) == crcAtInit_FLASH) {
        	periodicSTFailCount++;
		} else {
			periodicSTPassCount++;
		}
